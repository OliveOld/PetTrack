...
#### Request(App)
```
 [ Sync ][ Posture | Attribute ][ Value(4 byte) ]
```
6 byte. 현재 자세, 속성, 값을 전송한다.

#### Response(Device)
```
 [ Sync ][ Posture | Attribute ]
```
...
혹은 명시적으로 App 측에서 연결을 종료할 수 있다. 먼저 1 byte를 전송하는 측은 1 byte를 수신함으로써 서로 연결이 종료함을 알 수 있다.
